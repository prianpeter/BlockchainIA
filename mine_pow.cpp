...
// --- SHA256 C++ pur (source adaptée de B-Con crypto) ---
typedef struct {
    uint8_t data[64];
    uint32_t datalen;
...
// --- Minage C++ ---
py::tuple mine_pow(std::string base, int difficulty){
    unsigned long long nonce = 0;
    std::string hash;
    std::string prefix(difficulty,'0');
    while(true){
        hash = sha256_string(base + std::to_string(nonce));
        if(difficulty==0 || hash.substr(0,difficulty)==prefix) break;
        nonce++;
    }
...
PYBIND11_MODULE(mine_module, m){
    m.def("mine_pow",&mine_pow,"Mine a block with PoW (C++ SHA256 pure)");
}